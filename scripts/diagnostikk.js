...
module.exports = (robot) => {
  ...
  robot.respond(/hvilke rom er du i/, (msg) => {
    robot.http(`https://slack.com/api/channels.list?token=${process.env.HUBOT_SLACK_TOKEN}&exclude_archived=1`)
      .get()((err, res, body) => {
        const romnavn = JSON.parse(body).channels
    ...
    robot.http(`https://slack.com/api/groups.list?token=${process.env.HUBOT_SLACK_TOKEN}&exclude_archived=1`)
      .get()((err, res, body) => {
        const grupper = JSON.parse(body).groups
  ...
  robot.respond(/hjelp(?:\s+(.*))?/i, (res) => {
    ...
    let commands = robot.helpCommands().map(command => (
      command.replace(/^hubot/i, name)
    ));
